...
/**
 * Fills a `<datalist>` from a curated track menu: one tab-delimited `label<TAB>url` line per option.
 *
 * The two failure modes are not the same thing, and the point of this module is to keep them apart:
 * a 404 means the assembly simply has no curated menu for that dimension, which is expected and
 ...
 */
async function loadAnnotationDatalist(datalist, url, { load = loadString, presentAlert = message => AlertSingleton.present(message) } = {}) {
    ...
    try {
        ...
        if (404 === e.status) {
            // Expected: not all assemblies have track menus.
            console.warn(`No track menu found ${ url }`)
        } else {
            console.error(`Error loading track menu: ${ url } ${ e.message }`)
            presentAlert(`Error loading track menu: ${ url } ${ e.message }`)
        }
        return
    }
